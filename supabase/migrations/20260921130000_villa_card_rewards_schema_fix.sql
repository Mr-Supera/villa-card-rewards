-- Villa Card Rewards — corrected/re-runnable Supabase schema
-- Safe to apply after a failed/partial previous migration.
...
drop policy if exists "clientes_update_own_or_staff" on public.clientes;
create policy "clientes_update_own_or_staff"
...
drop policy if exists "cartoes_insert_own_or_staff" on public.cartoes;
create policy "cartoes_insert_own_or_staff"
on public.cartoes for insert to authenticated
with check (cliente_id = auth.uid() or public.has_staff_permission('cartoes_write'));

drop policy if exists "cartoes_update_own_or_staff" on public.cartoes;
create policy "cartoes_update_own_or_staff"
